...
expect <<EOF
  ...
  expect "#"  {send "sed -i '3s!.*!baseurl=ftp://192.168.${wd}.254/centos-1804!' /etc/yum.repos.d/local.repo\r"}
  expect "#"  {send ""}
  expect eof
EOF
...
#scp /root/shell-the-practical-application/redis.sh /root/shell-the-practical-application/monitoring.sh /root/shell-the-practical-application/lnmp.sh /root/shell-the-practical-application/ip.sh root@${ip}:/root
...
#
##发送redis软件包
#scp -r /linux-soft/03/redis root@${ip}:/root
#
##重新管理虚拟机,安装mysql
#expect <<EOF
#  spawn virsh console ${clone}
